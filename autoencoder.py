...
if __name__ == "__main__":
    tensor = torch.randn((1, 1, 512,512)).to("cuda")
    
    en = Encoder(channels=4,channel_multipliers=[1,1],n_resnet_blocks=1,in_channels=1,z_channels=2)
    de = Decoder(channels=4,channel_multipliers=[1,1],n_resnet_blocks=1,out_channels=1,z_channels=2)
    model = Autoencoder(encoder = en,decoder=de,emb_channels=2,z_channels=2).cuda()
